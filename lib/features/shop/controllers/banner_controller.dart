    ...
    try {
      if (_userController.userRole == 'Gérant') {
        ...
        if (gerantEtablissement != null &&
            (gerantEtablissement.id?.isNotEmpty ?? false)) {
    ...
    try {
      ...
      if (isGerant) {
        ...
        if (etab != null && (etab.id?.isNotEmpty ?? false)) {
          final bannersEstab =
              await _bannerRepository.getBannersByEstablishment(etab.id!);
          final produits =
              await produitRepository.getProductsByEtablissement(etab.id!);
          final productIds =
              produits.map((p) => p.id).where((id) => id.isNotEmpty).toList();
          final bannersProducts =
              await _bannerRepository.getBannersByProductIds(productIds);
          final combined = [...bannersEstab, ...bannersProducts];
          combined.sort((a, b) => (b.createdAt ?? DateTime(0))
      ...
      if (isGerant) {
        ...
        if (gerantEtablissement != null &&
          ...
          if (selectedLinkType.value == 'establishment') {
            ...
            try {
              final produit =
                  await produitRepository.getProductById(selectedLinkId.value);
              if (produit == null ||
                  produit.etablissementId != gerantEtablissement.id) {
                TLoaders.errorSnackBar(
                    title: 'Produit invalide',
                    message:
                        'Ce produit n\'appartient pas à votre établissement');
                isLoading.value = false;
                return;
              }
            } catch (e) {
    ...
    try {
      ...
      if (!(isGerant || isAdmin)) {
        TLoaders.errorSnackBar(
          title: 'Permission refusée',
          message:
              'Seuls les administrateurs et gérants peuvent supprimer des bannières',
        );
        return;
      }
